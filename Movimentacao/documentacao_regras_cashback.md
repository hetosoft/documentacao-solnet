...
| **Deletar** | Remove a combinação selecionada da grid. |
...
A tela oferece a função **Clonar Registro** no menu de contexto da grid de busca — clique-direito sobre a regra desejada e escolha `Clonar Registro`. Útil para criar variações: clone a regra base, ajuste o percentual ou a Forma de Pagamento, e grave com nova Descrição.
...
1. Na grid de busca, localize a regra base (ex.: `CASHBACK CARTÃO 5%`).
...
| `Movimentos de Vendas` | `202` | Origem das vendas que disparam a geração do cashback. |
