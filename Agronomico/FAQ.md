...
Sim. O Sol.NET **não exige protocolo de NF-e** para emitir. A receita pode ser produzida antes da SEFAZ autorizar a NF, ou em vendas que não usam NF-e.

### O atalho F9 não está abrindo o receituário. Por quê?
O atalho só dispara se houver na movimentação **algum item vinculado a um formulado**. Se nenhum produto da venda exige receita, o F9 não tem o que abrir. Verifique o `Cadastro de Produtos` → aba "Receituário Agronômico".
...
Não pela tela de F9 com os mesmos itens. Cada item de movimento tem **uma** associação a receituário. Se a venda tem múltiplos produtos com receita, todos saem em **um único receituário com vários itens**.
...
Em `Movimentos > Agronômico > Receituário Agronômico`, filtre por cliente. Há um índice otimizado para isso (busca rápida).

### Como vejo o consumo de ART de um profissional?
Em `Cadastros > Gestão de ART/TRT`. A coluna `Saldo Atual` versus `Número Inicial`/`Final` mostra quantas receitas já foram consumidas.
