...
1. **Tab `Visualizar`** — grid de busca com filtros próprios (`Fixo/Variável`, `Natureza`, `Classificação`, e busca por `Código ou Descrição Contábil`). Linhas de **contas sintéticas** aparecem em **cinza** no grid (para diferenciar das analíticas).
