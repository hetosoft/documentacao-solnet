...
```mermaid
graph TD
    A[IVA Dual Brasileiro] --> B[CBS - Federal]
    A --> C[IBS - Estadual/Municipal]
    B --> D[Arrecadado pela União]
    C --> E[Arrecadado por Comitê Gestor]
    D --> F[Receita Federal administra]
    E --> G[Distribuído a Estados e Municípios]
```
